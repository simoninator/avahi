...
static DBusHandlerResult dbus_create_domain_browser_object(DomainBrowserInfo **dbi, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
    DomainBrowserInfo *i;
    static const DBusObjectPathVTable vtable = {
        NULL,
...
static DBusHandlerResult dbus_create_service_type_browser_object(ServiceTypeBrowserInfo **stbi, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
...
static DBusHandlerResult dbus_create_service_browser_object(ServiceBrowserInfo **sbi, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
...
static DBusHandlerResult dbus_create_async_service_resolver_object(AsyncServiceResolverInfo **sri, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
    int32_t interface, protocol, aprotocol;
    uint32_t flags;
    char *name, *type, *domain;
...
static DBusHandlerResult dbus_create_async_host_name_resolver_object(AsyncHostNameResolverInfo **hri, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
    int32_t interface, protocol, aprotocol;
    uint32_t flags;
    char *name;
...
static DBusHandlerResult dbus_create_async_address_resolver_object(AsyncAddressResolverInfo **ari, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
    int32_t interface, protocol;
    uint32_t flags;
    char *address;
...
static DBusHandlerResult dbus_create_record_browser_object(RecordBrowserInfo **rbi, DBusConnection *c, DBusMessage *m, DBusError *error) {
    Client *client;
    RecordBrowserInfo *i;
    static const DBusObjectPathVTable vtable = {
        NULL,
...
static DBusHandlerResult msg_server_impl(DBusConnection *c, DBusMessage *m, AVAHI_GCC_UNUSED void *userdata) {
    ...
    if (dbus_message_is_method_call(m, DBUS_INTERFACE_INTROSPECTABLE, "Introspect"))
        ...
        r = dbus_create_domain_browser_object(&db, c, m, &error);
        avahi_s_domain_browser_start(db->domain_browser);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "DomainBrowserNew2")) {
        DomainBrowserInfo *db = NULL;
        r = dbus_create_domain_browser_object(&db, c, m, &error);
        return r;
        ...
        r = dbus_create_service_type_browser_object(&stbi, c, m, &error);
        avahi_s_service_type_browser_start(stbi->service_type_browser);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "ServiceTypeBrowserNew2")) {
        ServiceTypeBrowserInfo *stbi = NULL;
        r = dbus_create_service_type_browser_object(&stbi, c, m, &error);
        return r;
        ...
        r = dbus_create_service_browser_object(&sbi, c, m, &error);
        avahi_s_service_browser_start(sbi->service_browser);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "ServiceBrowserNew2")) {
        ServiceBrowserInfo *sbi = NULL;
        r = dbus_create_service_browser_object(&sbi, c, m, &error);
        return r;
        ...
        r = dbus_create_async_service_resolver_object(&sri, c, m, &error);
        avahi_s_service_resolver_start(sri->service_resolver);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "ServiceResolverNew2")) {
        AsyncServiceResolverInfo *sri = NULL;
        r = dbus_create_async_service_resolver_object(&sri, c, m, &error);
        return r;
        ...
        r = dbus_create_async_host_name_resolver_object(&hri, c, m, &error);
        avahi_s_host_name_resolver_start(hri->host_name_resolver);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "HostNameResolverNew2")) {
        AsyncHostNameResolverInfo *hri = NULL;
        r = dbus_create_async_host_name_resolver_object(&hri, c, m, &error);
        return r;
        ...
        r = dbus_create_async_address_resolver_object(&ari, c, m, &error);
        avahi_s_address_resolver_start(ari->address_resolver);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "AddressResolverNew2")) {
        AsyncAddressResolverInfo *ari = NULL;
        r = dbus_create_async_address_resolver_object(&ari, c, m, &error);
        return r;
        ...
        r = dbus_create_record_browser_object(&rbi, c, m, &error);
        avahi_s_record_browser_start_query(rbi->record_browser);
        return r;

    } else if (dbus_message_is_method_call(m, AVAHI_DBUS_INTERFACE_SERVER, "RecordBrowserNew2")) {
        RecordBrowserInfo *rbi = NULL;
        r = dbus_create_record_browser_object(&rbi, c, m, &error);
        return r;
    }
